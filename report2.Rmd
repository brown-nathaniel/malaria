...
model1sub = with(temp, glm(Y ~ AGE + GREEN + PHC, family="binomial"))
...
pool.compare(model1, model1sub, data=temp, method="likelihood")
...
dat_complete_matrix <- head(as.matrix(model.matrix(Y ~ as.factor(AGE) + as.factor(GREEN) + as.factor(PHC) + as.factor(BEDNET), data=dat_complete)))
fitvals <- t(as.matrix(pool_coef)) %*% t(dat_complete_matrix)
resids <- 
